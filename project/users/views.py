...
class LoginView(View):
    ...
    def get(self,request):
        if request.session.get('user_id'):
            ...
            active_user = User.objects.filter(id=active_user_id)
            if active_user:
                active_user.calculate_win_percentage()
...
class  LeaderboardView(View):
    template = 'users/leaderboard.html'
    ...
    def get(self,request,username):
        ...
        if active_user:
            ...
            if active_user_id == user.id:
                return render(request,self.template, {'active_user':active_user,'profile_form':profile_form,'user':user})
            return render(request,self.template,{'user':user, 'active_user':active_user})
        return redirect('/users/login')
    ...
    def post(self,request,username):
        ...
        if active_user_id == user.id:
            ...
            if updated_form.is_valid():
                active_user.about = updated_form.cleaned_data.get('about')
                active_user.calculate_win_percentage()
                active_user.save()
